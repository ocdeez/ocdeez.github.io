...
1. The latest browser-check result and freshness.
...
The full records remain available in `runs.jsonl` and `archive/runs.jsonl` even when the dashboard initially shows only the latest 25 rows. The page's load-more controls expose the rest of the stored history.
